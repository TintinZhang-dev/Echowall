...
2. **改 `site.config.json`**，至少改这几个字段：
   ...
   ```json
   {
     "school":   { "name": "某某中学", "shortName": "某某", "hasClassSystem": true },
...
4. **验证**：`curl https://<domain>/api/site-config` 应返回新学校名；刷新页面，标题/页头/页脚/主题色全部变。

## 四、字段说明

| 字段 | 说明 |
|---|---|
| `school.name` / `shortName` | 全名与简称；`hasClassSystem` 控制「届/班级」登录形态 |
